...
- **游标分页**：内置基于游标的分页查询 `QueryCursor`，返回 Go 1.23+ `iter.Seq2` 迭代器，支持对大数据集进行内存高效的流式遍历。支持 Gorm（行值表达式）、MongoDB（`$gt` 复合条件）和 ElasticSearch（`search_after` API）。在 ElasticSearch 游标场景中支持 PIT，并推荐与 `search_after` 搭配以提升数据变化过程中的一致性；可通过 `SetNeedPagination(false)` 自动启用，并用 `SetPitKeepAlive(...)` 配置保活时长。
...
b.SetLimit(100)
b.SetNeedPagination(false)         // ES 游标模式自动启用 PIT
b.SetPitKeepAlive(2 * time.Minute) // 配置 PIT keep_alive 时长
